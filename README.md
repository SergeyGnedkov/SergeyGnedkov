<table>
<tr>
<td width="75%" valign="top">

## 🇷🇺 RUS

# Привет! Я Сергей — Front-end разработчик и UI/UX дизайнер.

Создаю современные, адаптивные и удобные веб-интерфейсы.  
Объединяю дизайн и код для создания стильных и функциональных проектов.

---

## 🇬🇧 ENG

# Hi! I'm Sergey — Front-end Developer & UI/UX Designer.

I create modern, responsive and user-friendly web interfaces.  
I combine design and code to build clean and functional digital products.
---

### 🚀 Навыки/Skills
<p>
  <img src="https://img.shields.io/badge/HTML-111111?style=for-the-badge&logo=html5" />
  <img src="https://img.shields.io/badge/CSS-111111?style=for-the-badge&logo=css3" />
  <img src="https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript" />
  <img src="https://img.shields.io/badge/Front--End-111111?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Figma-111111?style=for-the-badge&logo=figma" />
  <img src="https://img.shields.io/badge/Photoshop-111111?style=for-the-badge&logo=adobephotoshop" />
  <img src="https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github" />
</p>

</td>

<td width="25%" valign="top">

# 📬 Контакты / Contacts

### GMAIL
📧 sergeygnedkovvv@gmail.com


### TG
💬 @SpasiI_Sohrani

</td>
</tr>
</table>
